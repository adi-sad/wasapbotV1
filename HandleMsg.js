            ...
            if (isOwnerBot) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          ...
                          await aruga.sendText(chekgrup.id, `Hai minna~, Im Hitman Bot. To find out the commands on this Bot type ${prefix}menu`)
                      })
            } else {
    ...
    if (!isGroupMsg) return aruga.reply(from, menuId.textPrem())
        ...
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) aruga.sendText(idk, `〘 *H I T M A N  B C* 〙\n\n${msg}`)
                if (cvk.isReadOnly) aruga.sendText(idk, `〘 *H I T M A N  B C* 〙\n\n${msg}`)
            }
